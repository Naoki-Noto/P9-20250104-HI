...
# 最初に設定した計算条件ごとのループ作成    
for target_data_name, (es_name, es), ne, k, selection_order, top_k, epsilon in product(
    target_data_names, es_dict.items(), ne_list, k_list, selection_order_list, top_k_list, epsilon_list
):
    ...
    for i in reversed(range(60)):               
        ...
        # 最初はすべてのデータをソースドメインに用いる
        source_data_names = All_source_data_names
        ...
        env = CatalystEnvironment(features=Xt_np, rewards=yt_np, top_k=top_k)
        state = env.reset()
        ...
        for step in range(59):
            ...
            Xs, ys, Xt, yt = selection.split_df(
                source_data_names=source_data_names,
                target_data_name=target_data_name
                )
    ...
    df.to_csv(f'../results/TrAdaBoostR2_Qtop{top_k}_ep{epsilon}_{es_name}_ne{ne}_{target_data_name}_source{selection_order}{k}_check_result.csv', index=False)
