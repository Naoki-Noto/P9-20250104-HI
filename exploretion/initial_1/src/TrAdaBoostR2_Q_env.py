...
class CatalystEnvironment:
    ...
    def select_action(self, model, q_network):
        ...
        if self.count == 0:  # 1手目の場合
            ...
            with torch.no_grad():
                state_tensor = torch.FloatTensor(self.get_remaining_features()[top_k_indices_in_remaining])  # 上位5つを入力
                q_values = q_network(state_tensor)
    ...
    def select_epsilon_action(self,epsilon_lower, model, q_network):
        """予測収率の下位epsilon_lower個の触媒のQ値を求め、最大の触媒のインデックスを返す。"""

        predicted_rewards = model.predict(pd.DataFrame(self.get_remaining_features()))
         # predicted_rewards を 1次元配列に変換
        predicted_rewards_1d = predicted_rewards.ravel()

        lower_remaining_indices = np.argsort(predicted_rewards_1d)[epsilon_lower:]
        lower_indices = [self.remaining_indices[i] for i in lower_remaining_indices]

      
        with torch.no_grad():
            state_tensor = torch.FloatTensor(self.get_remaining_features()[lower_remaining_indices])  # 上位5つを入力
            q_values = q_network(state_tensor)
            action = torch.argmax(q_values).item()  # 選択した触媒のtop_k_indices内のインデックスをスカラーで出力

         # 選択した触媒のインデックスを得る
            best_catalyst_index_in_lower = action
            best_catalyst_remaining_index = int(lower_remaining_indices[best_catalyst_index_in_lower])
            best_catalyst_actual_index = int(lower_indices[best_catalyst_index_in_lower])
