...
All_data_names = ['Reaction_CO_1.5h', 'Reaction_CO_7.5h', 'Reaction_CO_biphenyl', 'Reaction_CO_ortho',
                  ...
                  'Reaction_CN',
                  'Reaction_2+2',
                  'Reaction_CF3', 'Reaction_CH2CF3', 'Reaction_CH2F',
                  'Reaction_Cy', 
                  'Reaction_OCH2F', 'Reaction_SCF3',
                 'Reaction_Masuda', 'Reaction_Sumida'
                 ]

# 標準化する列名
features_to_standardize = ['k_ems(S1_S0)','k_ic(S1_S0)','k_isc(S1_T1)','k_risc(T1_S1)','k_isc(T1_S0)','ratio_k_ems(S1_S0)','ratio_k_ic(S1_S0)',
                           'ratio_k_isc(S1_T1)','ratio_k_risc(T1_S1)','ratio_k_isc(T1_S0)','HOMO','LUMO','E_S1','f_S1','E_T1','dEST(v)','dDM']
...
for data_name in All_data_names:
    ...
    dfs[f'{data_name}'][features_to_standardize] = scaler.fit_transform(dfs[f'{data_name}'][features_to_standardize])
...
for target in All_data_names:
    ...
    for random_state in range(100):
        ...
        env = CatalystEnvironment(features=features, rewards=rewards, top_k=5)
        state = env.reset()
    ...
    df.to_csv(f'../results/LinearRegression_Qtop5_{target}_standard_result.csv', index=False)
