...
for target in All_data_names:
    ...
    for random_state in range(100):
        ...
        # 線形回帰モデルの初期化
        #model = xgb.XGBRegressor(n_estimators=100, learning_rate=0.1, max_depth=8, subsample=0.8, colsample_bytree=0.7, gamma=0, reg_alpha=0.1, reg_lambda=1, random_state=42)
        model = xgb.XGBRegressor(random_state=42) #results_name "XGBRegression_result.csv"
    ...
    df.to_csv(f'../results/XGBRegression_{target}_result.csv', index=False)
